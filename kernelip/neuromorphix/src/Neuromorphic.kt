...
//    val NN_model_params: SnnArch
    ...
    // «внешний» сигнал тика — сделаем глобалом; при желании можно сделать портом OUT
    val tick = g.uport("tick", PORT_DIR.OUT, hw_dim_static(1), "0")

    // компоненты БММ будут добавляться по мере готовности
    private val tg = TickGen("tg0")

    init {
        // пример: тик раз в 1 мс при clk = 100 МГц (10 нс)
        tg.emit(
            g = g,
            cfg = TickGenCfg(timeslot = 1, unit = TimeUnit.MS, clkPeriodNs = 10),
            tickSignal = tick
        )

        // здесь позже добавим:
        // - PhaseFsm (использует tick)
        // - Queues / Selector / MemIf
        // - SynapticPhaseUnit / NeuronalPhaseUnit
    }

    /** Точка сборки готового дизайна */
    fun build(): Generic = g
}
