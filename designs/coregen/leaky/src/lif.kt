...
class lif_snn(name: String, nn_type : NEURAL_NETWORK_TYPE,  presyn_neur : Int,  postsyn_neur : Int,  weight_width : Int, potential_width : Int)
    : snn_arch(name, nn_type, presyn_neur, postsyn_neur, weight_width, potential_width, 2, 15, SPIKE_TYPE.BINARY) {
...
class lif(name : String, snn : lif_snn, tick_timeslot: Int) : Neuromorphic( name, snn, tick_timeslot) {
    ...
    constructor(name : String, snn : lif_snn) : this(name, snn, 15)

//    var input_if =  i_if("input_scheduler", snn.presyn_neur)  // input buffer
//    var output_if =  o_if("output_scheduler", snn.postsyn_neur)
